...
export default function EditRekomendasiEvent({
  params,
}: {
  ...
  useEffect(() => {
    const fetchRecommendation = async () => {
      ...
      try {
        ...
        setSlug(data.slug);
        setDesc(data.desc || '');
        setBannerUrl(data.banner_imagekit_url || '');
        
        // Parse detail_event metadata
        if (data.detail_event) {
          setOrganizer(data.detail_event.organizer || '');
          setEventDate(data.detail_event.event_date || '');
          setLocation(data.detail_event.location || '');
          setPatungan(data.detail_event.patungan !== undefined ? data.detail_event.patungan : '');
        }
      } catch (err: any) {
        showToast(err.message || 'Gagal memuat detail rekomendasi.', 'error');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Gagal memperbarui rekomendasi event.');
      }

      showToast('Rekomendasi event berhasil diperbarui.', 'success');
      router.push('/superadmin/rekomendasi-event');
      router.refresh();
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <div className="p-12 text-center text-slate-400 flex flex-col items-center justify-center gap-3">
        ...
        <span className="text-sm font-medium">Memuat detail rekomendasi...</span>
      </div>
    );
  }
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800 tracking-tight">Edit Rekomendasi Event</h1>
          <p className="text-slate-400 text-xs mt-0.5">Ubah rincian informasi rekomendasi event pilihan Anda.</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  rows={4}
                  placeholder="Deskripsikan event rekomendasi..."
                  value={desc}
