...
export default function CreateImagekitApi() {
  const { showToast } = useToast();
  const router = useRouter();
  const [name, setName] = useState('Default Imagekit');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('Kredensial Imagekit API berhasil dibuat.', 'success');
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800 tracking-tight">Tambah Kredensial Imagekit</h1>
          <p className="text-slate-400 text-xs mt-0.5">Daftarkan koneksi API Imagekit.io yang baru.</p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="Masukkan nama koneksi (contoh: Main Storage)..."
              value={name}
