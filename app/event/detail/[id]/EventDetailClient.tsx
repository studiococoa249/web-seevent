...
export default function EventDetailClient({
  ...
  const organizerAvatar = event.users?.profile?.profile_url_imagekit || getAvatarPlaceholder(event.users?.id, event.users?.nama_lengkap);
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm space-y-4">
              ...
              <p className="text-slate-600 text-sm leading-relaxed whitespace-pre-wrap">
                {event.desc_full || 'Tidak ada deskripsi tambahan.'}
              </p>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
              ...
              {participants.filter(p => p.status === 'Confirmed').length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {participants.filter(p => p.status === 'Confirmed').map((part) => {
                    const avatarUrl = part.users?.profile?.profile_url_imagekit || getAvatarPlaceholder(part.users?.id, part.users?.nama_lengkap);
                    return (
